...
task :deploy do
  ...
  deploy do
    ...
    command %{export PATH=/usr/local/rvm/gems/ruby-3.2.0/bin:$PATH}
    command %{export PATH=/usr/local/rvm/gems/ruby-3.2.0@global/bin:$PATH}
    command %{export PATH=:/usr/local/rvm/rubies/ruby-3.2.0/bin:$PATH}
